...
( ( T ) => {
    ...
    T.elements.custom( T.elements.TRL_PAGINATION, class extends HTMLElement {
        ...
        get maxItemsCount() {
            return this.hasAttribute( 'trl-max-items' ) 
                   ? parseInt( this.getAttribute( 'trl-max-items' ) ) 
                   : 0;
        }
        ...
        get itemsPerPageCount() {
            return this.hasAttribute( 'trl-items-per-page' ) 
                   ? parseInt( this.getAttribute( 'trl-items-per-page' ) ) 
                   : 0;
        }
        
        set currentPageOffset( count ) {
            if ( typeof count !== 'number' )
                throw new TypeError();
        ...
        get currentPageOffset() {
            return this.hasAttribute( 'trl-current-page' ) 
                   ? parseInt( this.getAttribute( 'trl-current-page' ) ) 
                   : 0;
        }
        ...
        getPage() {
            if (    this.__page === undefined           || 
                    this.__page === null                || 
                 !( this.__page instanceof HTMLElement )   ) {
        ...
        fetch() {
            ...
            if ( data !== null ) {
                ...
                var length = data.length < this.itemsPerPageCount 
                             ? data.length 
        ...
        get loading() {
            return this.hasAttribute( 'trl-loading' ) 
                   ? this.getAttribute( 'trl-loading' ) === 'true' 
                   : false;
        }
        ...
        setupPagination() {
            ...
            var num_pages = this.itemsPerPageCount >= this.maxItemsCount 
                            ? 1 
                            : Math.ceil( this.maxItemsCount / this.itemsPerPageCount );
